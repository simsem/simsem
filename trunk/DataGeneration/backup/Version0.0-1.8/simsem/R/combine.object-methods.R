...
setMethod("combine.object", signature(object1="freeParamSet", object2="list"), definition= function(object1, object2) {
		Tag <- object1@Tag
		exo <- (Tag == "SEM.exo") | (Tag == "Path.exo")
		nx <- ny <- ne <- nk <- 0
		if(Tag == "CFA") {
			ny <- nrow(object1@LY)
			ne <- nrow(object1@PS)
		...
		# } else {
			...
			# }
		# }
		return(new("reducedMatrixSet", Tag=object1@Tag, PS=object1@PS, AL=object1@AL, BE=object1@BE, PH=object1@PH, KA=object1@KA, GA=object1@GA,
			LY=object1@LY, TE=object1@TE, TY=object1@TY, LX=object1@LX, TD=object1@TD, TX=object1@TX, TH=object1@TH))
	}
)
#Arguments: object1 are freeParamSet.c of that saves all free parameters and values of fixed parameters
...
#Return: reducedMatrixSet.c containing parameter estimates or standard errors.
