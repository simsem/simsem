...
simSetPath <- function(..., exo = FALSE) {
	...
	if(7 %in% position) {
		...
		ifelse(4 %in% position, VE <- List[position == 4], ifelse(isNullObject(VPS[[1]]), { VE <- list(freeVector(1, ne)); comment(VE[[1]]) <- "default"}, VE <- list(new("NullSimVector"))))
	}
	ifelse(6 %in% position, ME <- List[position == 6], ME <- list(new("NullSimVector")))
	ifelse(5 %in% position, AL <- List[position == 5], ifelse(isNullObject(ME[[1]]), { AL <- list(freeVector(0, ne)); comment(AL[[1]]) <- "default"}, AL <- list(new("NullSimVector"))))
	Output <- NULL
	if(exo) {
		...
		if(12 %in% position) {
			...
			ifelse(10 %in% position, VPH <- List[position == 10], { VPH <- list(freeVector(1, nk)); comment(VPH[[1]]) <- "default"})
		}
		ifelse(11 %in% position, KA <- List[position == 11], { KA <- list(freeVector(0, nk)); comment(KA[[1]]) <- "default"})
