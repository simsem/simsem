...
#' @export
generate <- function(
  model, n, maxDraw = 50, misfitBounds = NULL, misfitType = "f0",
  ...
  if (inherits(model, "SimSem")) {
    ...
    data <- generateSimSem(
      model = model, n = n, maxDraw = maxDraw, misfitBounds = misfitBounds, misfitType = misfitType,
...
generateSimSem <- function(
  ...
  draws <- draw(model,
    maxDraw = maxDraw, misfitBounds = misfitBounds, misfitType = misfitType,
  ...
  datal <- mapply(
    FUN = createData, draws, indDist, facDist, errorDist, n = n, realData = realDataGroup,
...
popMisfitParams <- function(psl, df = NULL, covData = NULL) {
  ...
  misfit <- popMisfitMACS(paramM = lapply(macsreal, "[[", 1), paramCM = lapply(
    macsreal,
    "[[", 2
  ), misspecM = lapply(macsmis, "[[", 1), misspecCM = lapply(
    macsmis,
    "[[", 2
...
HeadrickSawilowsky1999 <- function(n = 100L, COR, skewness, kurtosis) {
  ...
  if (p == 2) {
    ...
    out <- nlminb(
      start = targetR, objective = objFUN2,
      scale = 10, control = list(trace = 0),
    ...
    start <- nlminb(
      start = runif(3, -1, 1), objective = findStartA,
      scale = 10, control = list(trace = 0),
    ...
    out <- nlminb(
      start = start$par, objective = objFUN3,
      scale = 10, control = list(trace = 0),
    ...
    start <- nlminb(
      start = runif(p + 1, -1, 1), objective = findStartB,
      scale = 10, control = list(trace = 0),
    ...
    out <- nlminb(
      start = start$par, objective = objFUN4,
      scale = 10, control = list(trace = 0),
...
output_template <- function(dfs, nrows) {
  ...
  list(setters = lapply(output, `[[`, "set"), getters = lapply(
    output,
    `[[`, "get"
  ))
}
