...
impose <- function(miss, data.mat, pmMCAR = NULL, pmMAR = NULL) {
  ...
  if (is.list(data.mat) && !is.data.frame(data.mat)) {
    if (!("data" %in% names(data.mat))) {
      stop("The list does not contain any dataset.")
    }
    data.mat$data <- as.data.frame(imposeMissing(data.mat$data,
      cov = miss@cov,
    ...
    data.mat <- as.data.frame(imposeMissing(data.mat,
      cov = miss@cov, pmMCAR = miss@pmMCAR,
...
imposeMissing <- function(data.mat, cov = 0, pmMCAR = 0, pmMAR = 0,
  ...
  if (nforms != 0 | !isTRUE(all.equal(twoMethod, 0))) {
    ...
    log.matpl <- plannedMissing(dim(data.mat), cov,
      nforms = nforms, twoMethod = twoMethod,
  ...
  if (any(prAttr > 0)) {
    log.mat3 <- attrition(data.mat,
      prob = prAttr, timePoints, cov, threshold,
      ignoreCols
    )
...
attrition <- function(data, prob = NULL, timePoints = 1,
  ...
  if (length(prob) != timePoints) {
    warning(
      "The specified number of timepoints (", timePoints,
...
plotLogitMiss <- function(script, ylim = c(0, 1), x1lim = c(-3, 3),
  ...
  for (i in 1:length(ivsep)) {
    ...
    if (length(temp) == 1) {
      ...
      plot(iv1, predVal,
        type = "n", xlab = ivname[1], ylab = "Missing Proportion",
      ...
      if (useContour) {
        contour(iv1, iv2, zpred,
          xlab = ivname[1], ylab = ivname[2],
          main = title
        )
      } else {
        persp(iv1, iv2, zpred,
          zlim = ylim, theta = 30, phi = 30,
