...
plotCIwidth <- function(object, targetParam, assurance = 0.50, useContour = TRUE) {
  ...
  if (sum(condition) == 0) {
    ...
    plotCutoffDataFrame(Data, cutoff,
      revDirec = FALSE, usedFit = targetParam, vector1 = condValue[, condition],
      nameVector1 = colnames(condValue)[condition], alpha = 1 - assurance
    )
  } else if (sum(condition) == 2) {
    condValue <- condValue[, condition]
    plotCutoffDataFrame(Data, cutoff,
      revDirec = FALSE, usedFit = targetParam, vector1 = condValue[, 1], vector2 = condValue[
        ,
        2
