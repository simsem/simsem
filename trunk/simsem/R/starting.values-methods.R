...
setMethod("starting.values", signature(object="SimSet"), definition=function(object, trial, reduced=FALSE) {
		result <- run(object)
		#browser()
		if(trial > 1) {
			for(i in 2:trial) {
...
#Arguments: 
#	object:		SimSet.c used to find starting values
# 	trial:		Number of random draws before taking the average
...
#Description: 	Draw samples from SimSet.c, take the average, and report the starting values as a MatrixSet.c.
