...
getPowerFitDataFrame <- function(altObject, cutoff, revDirec = FALSE,
  ...
  common.name <- Reduce(intersect, list(
    colnames(altObject), names(cutoff),
    usedFit
  ))
  ...
  for (i in 1:length(common.name)) {
    temp[i] <- pValueVector(
      target = as.numeric(cutoff[i]), dist = as.vector(altObject[
        ,
        i
      ]), revDirec = revDirec, x = predictor, xval = predictorVal, df = df,
...
getPowerFitCutoff <- function(altObject, cutoff, revDirec = FALSE,
  ...
  output <- getPowerFitDataFrame(Data, cutoff, revDirec, usedFit,
    predictor = condValue,
...
getPowerFitNullObj <- function(altObject, nullObject, revDirec = FALSE,
  ...
  if (is.null(condValue)) {
    ...
    varyingCutoff <- getCutoffDataFrame(
