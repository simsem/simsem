...
getKeywords <- function() {
    ...
    loading <- c(LY, "loading", "factor loading")
    #errorCov <- c(TE, TD, "error.cov", "errorcov", "error covariance")
    errorCov <- c(TE, "error.cov", "errorcov", "error covariance")
    # errorCor <- c(RTE, RTD, "error.cor", "errorcor", "error correlation")
    errorCor <- c(RTE,"error.cor", "errorcor", "error correlation")
    #errorVar <- c(VTD, VTE, "error.var", "errorVar", "Variance of Measurement Error")
    errorVar <- c(VTE, "error.var", "errorVar", "Variance of Measurement Error")
    #indicatorVar <- c(VX, VY, "indVar", "ind.var", "indicator.var", "indicatorVar", "Variance of Indicators")
    indicatorVar <- c(VY, "indVar", "ind.var", "indicator.var", "indicatorVar", "Variance of Indicators")
    #indicatorMean <- c(MX, MY, "indMean", "ind.mean", "indicator.mean", "indicatorMean", "means of Indicators")
    indicatorMean <- c(MY, "indMean", "ind.mean", "indicator.mean", "indicatorMean", "means of Indicators")
    #facCov <- c(PS, PH, "latent.cov", "latentCov", "factor.cov", "factorCov", "faccov", "factor covariance")
    facCov <- c(PS,"latent.cov", "latentCov", "factor.cov", "factorCov", "faccov", "factor covariance")
    ##facCor <- c(RPS, RPH, "latent.cor", "latentCor", "factor.cor", "factorCor", "faccor", "factor correlation")
    facCor <- c(RPS,"latent.cor", "latentCor", "factor.cor", "factorCor", "faccor", "factor correlation")
    #intercept <- c(TY, TX, "intercept", "measurement intercept")
    intercept <- c(TY,"intercept", "measurement intercept")
    ...
    facVar <- c(VE, VPS,"latent.var", "latentVar", "factor.var", "factorVar", "facvar", "factor variance")
    ...
    #    optNone = optNone)
    result <- list(LY = LY, TE = TE, RTE = RTE, PS = PS, RPS = RPS, BE = BE, VY = VY, VPS = VPS, VE = VE, TY = TY, ME = ME, VTE = VTE, AL = AL, MY = MY, 
        GA = GA, KA = KA, loading = loading, errorCov = errorCov, errorCor = errorCor, errorVar = errorVar, indicatorVar = indicatorVar, 
        indicatorMean = indicatorMean, facCov = facCov, facCor = facCor, intercept = intercept, facMean = facMean, facVar = facVar, usedFit = usedFit, usedFitPop = usedFitPop, optMin = optMin, optMax = optMax, 
        optNone = optNone)
    return(result)
} 
