...
runLavaan <- function(object, Data) {
	...
	Model.summary <- extract.lavaan.summary(Output)
	Converged <- Fit@Fit@converged
        return(new("simAnalysis", Parameters=object@Parameters, Starting.Values=object@Starting.Values,
                   Constraint=object@simConstraint, Program=object@Program, Estimates=as.vector(0),
                   Fit=Fit, SE=as.vector(0), Convergence=Converged))
}
