...
#' path.A[3, 2] <- NA
#' model.A <- estmodel(
#'   LY = loading, BE = path.A, modelType = "SEM",
...
#' path.B[2, 3] <- NA
#' model.B <- estmodel(
#'   LY = loading, BE = path.B, modelType = "SEM",
#'   indLab = c(paste("x", 1:3, sep = ""), paste("y", 1:8, sep = ""))
#' )
#'
...
pValueNonNested <- function(
  ...
  result2 <- NULL
  if (any(condition)) {
    result1 <- pValueDataFrame(cutoff, Data1, revDirec,
      x = condValue, xval = predictorVal,
      df = df, asLogical = FALSE
    )
    names(result1) <- usedFit
    result2 <- pValueDataFrame(cutoff, Data2, !revDirec,
      x = condValue, xval = predictorVal,
      df = df, asLogical = FALSE
    )
