# plotCutoffNonNested: plot the cutoff from the difference in fit indices from two models

plotCutoffNonNested <- function(dat1Mod1, dat1Mod2, dat2Mod1=NULL, dat2Mod2=NULL, alpha=0.05, cutoff = NULL, usedFit = NULL, useContour = T, onetailed=FALSE) {
	...
	cutoff2 <- NULL
	cutoff3 <- NULL
	if(!is.null(cutoff)) cutoff3 <- -cutoff
	cutoff4 <- NULL
    if (!is.null(alpha)) {
        if (all(!condition)) {
            if(is.null(cutoff)) {
				...
				cutoff2 <- cutoff[2,]
				if(!is.null(dat2Mod1) && !is.null(dat2Mod2)) {
					cutoff <- getCutoffNonNested(dat2Mod2, dat2Mod1, alpha=alpha, onetailed=onetailed)[[1]]
					cutoff3 <- -cutoff[1,]
					cutoff4 <- -cutoff[2,]
				}
			} 
    ...
    if (sum(condition) == 0) {
		if(!is.null(dat2Mod1) && !is.null(dat2Mod2)) {
			plotOverHist(Data2, Data1, cutoff=cutoff1, usedFit=usedFit, cutoff2=cutoff2, cutoff3=cutoff3, cutoff4=cutoff4)
		} else {
			plotCutoff(Data1, cutoff1, usedFit=usedFit, cutoff2=cutoff2)
		}
    } else if (sum(condition) == 1) {
        plotCutoff(Data2, cutoff, FALSE, usedFit, vector1 = condValue[, condition], nameVector1 = colnames(condValue)[condition], alpha = alpha)
    } else if (sum(condition) == 2) {
        condValue <- condValue[, condition]
        plotCutoff(Data2, cutoff, FALSE, usedFit, vector1 = condValue[, 1], vector2 = condValue[, 2], nameVector1 = colnames(condValue)[1], 
