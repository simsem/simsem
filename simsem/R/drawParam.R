...
#' @export
draw <- function(
  model, maxDraw = 50, misfitBounds = NULL, averageNumMisspec = FALSE,
  ...
  drawParam(model@dgen,
    maxDraw = maxDraw, numFree = max(model@pt$free), misfitBounds = misfitBounds,
...
#' @keywords internal
drawParam <- function(
  paramSet, maxDraw = 50, numFree = 1, misfitBounds = NULL, averageNumMisspec = FALSE,
  ...
  while (draw <= maxDraw) {
    if (misspec) {
      ...
      if (!is.null(optMisfit)) {
        ...
        if (!(all(is.finite(unlist(lapply(macsPopls, "[[", 2)))) && (sum(unlist(lapply(lapply(lapply(
          macsPopls,
          "[[", 2
        ), eigen), "[[", 1)) <= 0) == 0) && all(sapply(
          fullpls,
          validateObject
        )))) {
      ...
      if (all(sapply(fullpls, validateObject))) {
        ...
        if (!(all(is.finite(unlist(lapply(macsPopls, "[[", 2)))) && (sum(unlist(lapply(lapply(lapply(
          macsPopls,
          "[[", 2
        ), eigen), "[[", 1)) <= 0) == 0) && all(is.finite(unlist(lapply(
          macsMisls,
          "[[", 2
        )))) && (sum(unlist(lapply(lapply(lapply(
          macsMisls, "[[",
          2
        ), eigen), "[[", 1)) <= 0) == 0))) {
          # cat('FAIL')
        ...
        if (is.null(misfitBounds)) {
          ...
          misfit <- popMisfitMACS(paramM = lapply(macsPopls, "[[", 1), paramCM = lapply(
            macsPopls,
            "[[", 2
          ), misspecM = lapply(macsMisls, "[[", 1), misspecCM = lapply(
            macsMisls,
            "[[", 2
          ...
          if (!is.null(misfit) && (misfit > misfitBounds[1] & misfit < misfitBounds[2])) {
            ...
            if (draw > maxDraw) {
              stop(paste0(
                "Cannot obtain misfit in bounds within maximum number of draws. Last ",
                misfitType, ": ", misfit
              ))
            }
            next
      ...
      if (all(sapply(fullpls, validateObject))) {
        redpls <- lapply(fullpls, reduceMatrices)
        macsPopls <- mapply(createImpliedMACS, redpls, covStat, SIMPLIFY = FALSE)
        if (all(is.finite(unlist(lapply(macsPopls, "[[", 2)))) && all(unlist(lapply(lapply(lapply(
          macsPopls,
          "[[", 2
        ), eigen), "[[", 1)) > 0)) {
          break
  ...
  if (draw < maxDraw) {
    ...
    stop(paste0(
      "Cannot make a good set of parameters with given constraints within the maximum number of draws.\n",
...
#' @keywords internal
rawDraw <- function(
  simDat, constraint = TRUE, misSpec = TRUE, parMisOnly = FALSE,
  misOnly = FALSE
) {
...
fillParam <- function(rawParamSet, covStat = NULL) {
  ...
  fullParamSet <- list(
    LY = LY, VTE = VTE, TE = TE, RTE = RTE, VY = VY, TY = TY,
...
reduceMatrices <- function(paramSet) {
  ...
  reducedParamSet <- list(
    PS = paramSet$PS, BE = paramSet$BE, AL = paramSet$AL,
...
extractLab <- function(pls, dgen, fill = FALSE, con = NULL) {
  ...
  freetemp2 <- mapply(function(x, k) {
    names(x) <- paste0(names(x), "_", k)
    x
  }, x = freetemp1, k = seq_along(free), SIMPLIFY = FALSE)
  ...
  valtemp2 <- mapply(function(x, k) {
    names(x) <- paste0(names(x), "_", k)
    x
  }, x = valtemp1, k = seq_along(val), SIMPLIFY = FALSE)
