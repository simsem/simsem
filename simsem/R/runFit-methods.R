...
setMethod("runFit", signature(model = "SimModel"), definition = function(model, 
    realdata, nRep = 1000, misspec = new("NullSimMisspec"), maxDraw = 100, sequential = NA, 
    ...
    SimData <- simData(out, misspec = misspec, maxDraw = maxDraw, sequential = sequential, 
        facDist = facDist, errorDist = errorDist, indDist = indDist, usedStd = usedStd, 
        realData = realdata)
...
setMethod("runFit", signature(model = "SimModelOut"), definition = function(model, 
    ...
    SimData <- simData(model, misspec = misspec, maxDraw = maxDraw, sequential = sequential, 
        facDist = facDist, errorDist = errorDist, indDist = indDist, usedStd = usedStd, 
