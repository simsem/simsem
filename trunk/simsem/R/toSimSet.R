...
toSimSet <- function(out, usedStd = TRUE) {
    ...
    if (!isNullObject(param@LY)) 
        LY <- simMatrix(free = param@LY, param = start@LY)
    if (!isNullObject(param@TY)) 
        TY <- simVector(free = param@TY, param = start@TY)
    if (!isNullObject(param@BE)) 
        BE <- simMatrix(free = param@BE, param = start@BE)
    if (!isNullObject(param@AL)) 
        AL <- simVector(free = param@AL, param = start@AL)
    if (!isNullObject(param@TE)) 
        TE <- symMatrix(free = param@TE, param = start@TE)
    if (!isNullObject(param@PS)) 
        PS <- symMatrix(free = param@PS, param = start@PS)
    if (!isNullObject(param@LX)) 
        LX <- simMatrix(free = param@LX, param = start@LX)
    if (!isNullObject(param@TX)) 
        TX <- simVector(free = param@TX, param = start@TX)
    if (!isNullObject(param@GA)) 
        GA <- simMatrix(free = param@GA, param = start@GA)
    if (!isNullObject(param@KA)) 
        KA <- simVector(free = param@KA, param = start@KA)
    if (!isNullObject(param@TD)) 
        TD <- symMatrix(free = param@TD, param = start@TD)
    if (!isNullObject(param@PH)) 
        PH <- symMatrix(free = param@PH, param = start@PH)
    if (!isNullObject(param@TH)) 
        TH <- simMatrix(free = param@TH, param = start@TH)
    ...
    if (modelType == "CFA") {
        ...
        result <- simSetPath(BE = BE, AL = AL, PS = PS, GA = GA, KA = KA, PH = PH, 
            exo = TRUE)
