...
setMethod("pValue", signature(target = "numeric", dist = "data.frame"), 
    definition = function(target, dist, revDirec = NULL, asLogical = FALSE) {
...
setMethod("pValue", signature(target = "SimModelOut", dist = "SimResult"), 
    definition = function(target, dist, usedFit = NULL) {
        dist <- clean(dist)
        if (is.null(usedFit)) 
            usedFit <- getKeywords()$usedFit
        revDirec <- !(usedFit %in% c("CFI", "TLI"))
        logicalMat <- pValue(target@fit[usedFit], dist@fit[, usedFit], revDirec, 
            asLogical = TRUE)
