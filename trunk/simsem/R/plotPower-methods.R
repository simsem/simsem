# plotPower: This function will plot sampling distributions of fit indices that
# visualize power

setMethod("plotPower", signature(altObject = "data.frame", nullObject = "vector"), 
    definition = function(altObject, nullObject, usedFit = NULL) {
        plotCutoff(altObject, nullObject, usedFit = usedFit)
    })

setMethod("plotPower", signature(altObject = "SimResult", nullObject = "vector"), 
    definition = function(altObject, nullObject, usedFit = NULL) {
        altObject <- clean(altObject)
        plotCutoff(altObject@fit, nullObject, usedFit = usedFit)
    })

setMethod("plotPower", signature(altObject = "data.frame", nullObject = "data.frame"), 
    definition = function(altObject, nullObject, alpha, usedFit = NULL) {
        percentile <- 1 - alpha
...
setMethod("plotPower", signature(altObject = "SimResult", nullObject = "SimResult"), 
    definition = function(altObject, nullObject, alpha, usedFit = NULL) {
        altObject <- clean(altObject)
