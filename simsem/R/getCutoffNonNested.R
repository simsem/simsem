# getCutoffNonNested: get the cutoff from the simulated sampling distribution of difference in fit indices

# Compute fit1 - fit2
getCutoffNonNested <- function(dat1Mod1, dat1Mod2, dat2Mod1=NULL, dat2Mod2=NULL, alpha=.05, usedFit=NULL, onetailed=FALSE, nVal = NULL, pmMCARval = NULL, pmMARval = NULL, df = 0) { 
	if (is.null(usedFit)) 
	...
	if(!is.null(dat2Mod1) & !is.null(dat2Mod2)) {
		...
		if(!isTRUE(all.equal(unique(dat1Mod1@n), unique(dat1Mod2@n)))) stop("Models are based on different values of sample sizes")
		if(!isTRUE(all.equal(unique(dat1Mod1@pmMCAR), unique(dat1Mod2@pmMCAR)))) stop("Models are based on different values of the percent completely missing at random")
		if(!isTRUE(all.equal(unique(dat1Mod1@pmMAR), unique(dat1Mod2@pmMAR)))) stop("Models are based on different values of the percent missing at random")
	}
	...
	result <- list()
	if(onetailed) {
		cutoffDat1 <- getCutoff(Data1, alpha, FALSE, usedFit, predictor = condValue, predictorVal = predictorVal, df = df)
		...
		if(!is.null(dat2Mod1) & !is.null(dat2Mod2)) {
			cutoffDat2 <- getCutoff(Data2, 1 - alpha, FALSE, usedFit, predictor = condValue, predictorVal = predictorVal, df = df)
		...
		lower <- alpha / 2
		upper <- 1 - (alpha / 2)
		cutoffDat1Low <- getCutoff(Data1, lower, FALSE, usedFit, predictor = condValue, predictorVal = predictorVal, df = df)
		cutoffDat1High <- getCutoff(Data1, upper, FALSE, usedFit, predictor = condValue, predictorVal = predictorVal, df = df)
		resultModel1 <- rbind(cutoffDat1Low, cutoffDat1High)
		resultModel1 <- apply(resultModel1, 2, sort)
		rownames(resultModel1) <- c("lower", "upper")
		result$model1 <- resultModel1
		if(!is.null(dat2Mod1) & !is.null(dat2Mod2)) {
			cutoffDat2Low <- getCutoff(Data2, lower, FALSE, usedFit, predictor = condValue, predictorVal = predictorVal, df = df)	
			cutoffDat2High <- getCutoff(Data2, upper, FALSE, usedFit, predictor = condValue, predictorVal = predictorVal, df = df)
