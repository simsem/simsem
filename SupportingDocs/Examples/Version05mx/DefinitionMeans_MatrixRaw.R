...
covData <- data.frame(def=def)


defMeansFit <- mxRun(defMeansModel)

dat1 <- generateOpenMx(defMeansFit, n=1000, covData=covData)
dat2 <- generateOpenMx(defMeansModel, n=1000, covData=covData)

out1 <- analyzeOpenMx(defMeansFit, dat1)
out2 <- analyzeOpenMx(defMeansFit, dat2)
