...
test.bind <- function() {
    ...
    a[, 2] <- "a1"
    bind(free = a)
    bind(free = a, popParam = 0.7, misspec = 0.01)
    bind(free = a, popParam = "runif(1,0,1)", misspec = 0.01)
    bind(free = a, popParam = "runif(1,0,1)", misspec = "runif(1,0,1)")
    
    b <- c(NA, 0, "b1", "b1")
    bind(free = b)
    bind(free = b, popParam = 0.7, misspec = 0.01)
    bind(free = b, popParam = "runif(1,0,1)", misspec = 0.01)
