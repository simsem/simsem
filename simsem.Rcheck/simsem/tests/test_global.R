...
LX.trivial <- simMatrix(loading.X.trivial, "runif(1,-0.2,0.2)")
...
loading[25:36, 3] <- NA
loading[37:48, 4] <- NA
LY <- simMatrix(loading, "runif(1,0.5,0.9)")

faccor <- matrix(NA, 4, 4)
diag(faccor) <- 1
RPS <- symMatrix(faccor, "runif(1,.1,.6)")
...
diag(latent.cor) <- 1
RPH <- symMatrix(latent.cor, "runif(1,-.5,.5)")
