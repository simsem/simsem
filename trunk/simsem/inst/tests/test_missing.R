...
data <- matrix(rnorm(10000,1,1),ncol=20,nrow=10000)
dataC <- cbind(data,rnorm(10000,0,1))
dataE <- matrix(rnorm(10000,1,1),ncol=20,nrow=10000)
...
percentmis <- mean(replicate(10,sum(is.na(imposeMissing(dataC,covs=c(20,21),pmMCAR=.1)))/length(dataC[,1:19])))
expect_true((percentmis > .099) && (percentmis < .101))


context("MAR")
percentmis <- mean(replicate(10,sum(is.na(imposeMissing(dataC,covs=c(21,22),pmMAR=.1)))/length(dataC[,1:19])))
expect_true((percentmis > .099) && (percentmis < .101))

percentmis <- mean(replicate(10,sum(is.na(imposeMissing(dataCE,covs=c(21,22),pmMAR=.1)))/length(dataCE[,1:20])))
expect_true((percentmis > .099) && (percentmis < .101))
