...
simParamSEM <- function(..., exo = FALSE) {
	...
	if(exo) {
		ifelse(contain(9, position), GA <- List[position == 9][[1]], stop("No path coefficient object between factor.ETA and factor.KSI"))
		ne <- nrow(GA)
		nk <- ncol(GA)
		ifelse(contain(3, position), BE <- List[position == 3][[1]], BE <- matrix(0, ne, ne))
		ifelse(contain(4, position), AL <- List[position == 4][[1]], AL <- rep(NA, ne))
		ifelse(contain(6, position), PS <- List[position == 6][[1]], PS <- diag(NA, ne))
		ifelse(contain(10, position), KA <- List[position == 10][[1]], KA <- rep(NA, nk))
		ifelse(contain(12, position), PH <- List[position == 12][[1]], PH <- matrix(NA, nk, nk))
		ifelse(contain(1, position), LY <- List[position == 1][[1]], stop("No loading object of indicator.Y from factor.ETA in SEM"))
		ny <- nrow(LY)
		ifelse(contain(2, position), TY <- List[position == 2][[1]], TY <- rep(NA, ny))
		ifelse(contain(5, position), TE <- List[position == 5][[1]], TE <- diag(NA, ny))
		ifelse(contain(7, position), LX <- List[position == 7][[1]], stop("No loading object of indicator.X from factor.KSI in SEM"))
		nx <- nrow(LX)
		...
		ne <- nrow(BE)
		ifelse(contain(4, position), AL <- List[position == 4][[1]], AL <- rep(0, ne))
		PS <- NULL
		if(contain(6, position)) { 
