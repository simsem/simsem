...
#' @export
model <- function(
  LY = NULL, PS = NULL, RPS = NULL, TE = NULL, RTE = NULL, BE = NULL,
  ...
  paramSet <- list(
    LY = LY, PS = PS, RPS = RPS, TE = TE, RTE = RTE, BE = BE, VTE = VTE,
    ...
    if (length(mg) > 0 || ngroups > 1) {
      if (ngroups > 1 && (length(sg) == sum(!sapply(paramSet, is.null)))) {
        ...
        for (i in seq_along(sgidx)) {
          ...
          if (inherits(paramSet[sgidx][[i]], "SimMatrix")) {
            ...
            paramSet[sgidx][[i]] <- replicate(n, new("SimMatrix",
              free = temp@free,
              popParam = temp@popParam, misspec = temp@misspec, symmetric = temp@symmetric
            ))
          } else {
            temp <- paramSet[sgidx][[i]]
            paramSet[sgidx][[i]] <- replicate(n, new("SimVector",
              free = temp@free,
      ...
      for (i in seq_along(psl)) {
        if (i == 1) {
          ...
          pt <- mapply(pt, buildPT(psl[[i]],
            pt = pt, group = i, facLab = facLab,
            indLab = indLab, covLab = covLab
      ...
      return(new("SimSem",
        pt = pt, dgen = paramSet, modelType = modelType,
...
buildPT <- function(paramSet, pt = NULL, group = 1, facLab = NULL, indLab = NULL, covLab = NULL) {
  ...
  if (!is.null(paramSet$PS)) {
    ...
    if (!is.null(pt)) {
      if (!is.null(paramSet$LY)) {
        pt <- mapply(pt, parseFree(paramSet$PS,
          group = group, pt = pt, op = "~~",
          lhs, rhs
        ), FUN = c, SIMPLIFY = FALSE)
      } else {
        pt <- parseFree(paramSet$PS,
          group = group, pt = pt, op = "~~", lhs,
          rhs
        )
      }
    } else {
  ...
  if (!is.null(paramSet$RPS)) {
    ...
    if (!is.null(pt)) {
      if (!is.null(paramSet$LY)) {
        pt <- mapply(pt, parseFree(paramSet$RPS,
          group = group, pt = pt,
          op = "~~", lhs, rhs
        ), FUN = c, SIMPLIFY = FALSE)
      } else {
        pt <- parseFree(paramSet$RPS,
          group = group, pt = pt, op = "~~",
          lhs, rhs
        )
      }
  ...
  if (!is.null(paramSet$TE)) {
    ...
    if (!is.null(pt)) {
      pt <- mapply(pt, parseFree(paramSet$TE,
        group = group, pt = pt, op = "~~",
        lhs, rhs
      ), FUN = c, SIMPLIFY = FALSE)
    } else {
      pt <- parseFree(paramSet$TE,
        group = group, pt = pt, op = "~~", lhs,
        rhs
      )
    }
  ...
  if (!is.null(paramSet$RTE)) {
    ...
    pt <- mapply(pt, parseFree(paramSet$RTE,
      group = group, pt = pt, op = "~~",
      lhs, rhs
    ), FUN = c, SIMPLIFY = FALSE)
  }
  ...
  if (!is.null(paramSet$BE)) {
    ...
    pt <- mapply(pt, parseFree(paramSet$BE,
      group = group, pt = pt, op = "~",
      rhs, lhs
    ), FUN = c, SIMPLIFY = FALSE)
  }
  ...
  if (!is.null(paramSet$AL)) {
    ...
    pt <- mapply(pt, parseFree(paramSet$AL,
      group = group, pt = pt, op = "~1",
      lhs, rhs
    ), FUN = c, SIMPLIFY = FALSE)
  }
  ...
  if (!is.null(paramSet$TY)) {
    ...
    pt <- mapply(pt, parseFree(paramSet$TY,
      group = group, pt = pt, op = "~1",
      lhs, rhs
    ), FUN = c, SIMPLIFY = FALSE)
  }
  ...
  if (!is.null(paramSet$GA)) {
    ...
    pt <- mapply(
      FUN = c, pt, parseFree(paramSet$GA,
        group = group, pt = pt,
        op = "~", lhs = lhs, rhs = rhs
      ),
      SIMPLIFY = FALSE
  ...
  if (!is.null(paramSet$KA)) {
    ...
    pt <- mapply(
      FUN = c, pt, parseFree(paramSet$KA,
        group = group, pt = pt,
        op = "~", lhs = lhs, rhs = rhs
      ),
      SIMPLIFY = FALSE
  ...
  if (!is.null(covLab)) {
    ...
    pt <- mapply(pt, parseFree(bind(matrix(0, nz, nz), symmetric = TRUE),
      group = group, pt = pt, op = "~~",
      lhs, rhs, exo = 1, forceUstart = NA
    ), FUN = c, SIMPLIFY = FALSE)
    lhs2 <- covLab
    rhs2 <- rep("", times = nz)
    pt <- mapply(pt, parseFree(bind(rep(0, nz)),
      group = group, pt = pt, op = "~1",
      lhs2, rhs2, exo = 1, forceUstart = NA
    ), FUN = c, SIMPLIFY = FALSE)
  }
  return(pt)
}
...
#' @keywords internal
parseFree <- function(
  simDat, group, pt, op, lhs = NULL, rhs = NULL,
...
#' @export
model.cfa <- function(
  LY = NULL, PS = NULL, RPS = NULL, TE = NULL, RTE = NULL, VTE = NULL,
  ...
  model(
    LY = LY, PS = PS, RPS = RPS, TE = TE, RTE = RTE, BE = NULL, VTE = VTE,
...
#' @export
model.path <- function(
  PS = NULL, RPS = NULL, BE = NULL, VPS = NULL, VE = NULL, AL = NULL,
  ME = NULL, KA = NULL, GA = NULL, indLab = NULL, facLab = NULL, covLab = NULL, groupLab = "group", ngroups = 1, con = NULL
) {
  model(
    LY = NULL, PS = PS, RPS = RPS, TE = NULL, RTE = NULL, BE = BE, VTE = NULL,
...
model.sem <- function(
  ...
  model(
    LY = LY, PS = PS, RPS = RPS, TE = TE, RTE = RTE, BE = BE, VTE = VTE, VY = VY,
  ...
  if (modelType == "cfa") {
    ...
    if (is.null(PS)) {
      if (is.null(RPS)) {
        ...
        temp2 <- lapply(LY, function(y) {
          !apply(y@free, 2, function(x) {
            any(!is.free(x) &
              (!is.na(x) & x != 0))
          })
        })
...
#' @rdname estmodel
#' @export
estmodel.cfa <- function(
  LY = NULL, PS = NULL, RPS = NULL, TE = NULL, RTE = NULL,
  VTE = NULL, VY = NULL, VPS = NULL, VE = NULL, TY = NULL, AL = NULL, MY = NULL,
  ME = NULL, KA = NULL, GA = NULL, indLab = NULL, facLab = NULL, covLab = NULL, groupLab = "group", ngroups = 1, con = NULL
) {
  estmodel(
...
#' @rdname estmodel
#' @export
estmodel.path <- function(
  PS = NULL, RPS = NULL, BE = NULL, VPS = NULL, VE = NULL,
...
#'
#' @export
model.lavaan <- function(
  object, std = FALSE, LY = NULL, PS = NULL, RPS = NULL, TE = NULL,
  ...
  if (std) {
    ...
    freeUnstd <- labelFree(
      lavInspect(object, "free", list.by.group = FALSE),
    ...
    if (modelType %in% c("cfa", "sem")) {
      ...
      VY <- mapply(FUNV, x = rep(list(rep(NA, ny)), ngroups), y = rep(list(rep(
        1,
        ny
      )), ngroups), h = freeUnstd[names(freeUnstd) == "theta"], pttemp = ptg, MoreArgs = list(lhstemp = indLab, optemp = "~~"), SIMPLIFY = FALSE)
      if (!is.list(TY)) {
      ...
      TY <- mapply(FUNV,
        x = rep(list(rep(NA, ny)), ngroups), y = rep(list(rep(
          0,
          ny
        )), ngroups), z = TY, h = freeUnstd[names(freeUnstd) == "nu"], pttemp = ptg, MoreArgs = list(lhstemp = indLab, optemp = "~1"),
        SIMPLIFY = FALSE
      )
      if (!is.null(covLab)) {
      ...
      AL <- mapply(FUN,
        x = rep(list(rep(NA, ne)), ngroups), y = rep(list(rep(
          0,
          ne
        )), ngroups), z = AL, h = freeUnstd[names(freeUnstd) == "alpha"], pttemp = ptg, MoreArgs = list(lhstemp = indLab, optemp = "~1"),
        SIMPLIFY = FALSE
      )
      VE <- mapply(FUNV, x = rep(list(rep(NA, ne)), ngroups), y = rep(list(rep(
        1,
        ne
      )), ngroups), h = freeUnstd[names(freeUnstd) == "psi"], pttemp = ptg, MoreArgs = list(lhstemp = indLab, optemp = "~~"), SIMPLIFY = FALSE)
    }
    ...
    if (inherits(object, "lavaan.mi")) {
      ...
      est <- lavInspect(object, "est", list.by.group = FALSE)
    }
    if (!is.null(covLab)) est <- reshuffleParamGroup(est, covLab, indLab, facLab, ngroups)
    free <- labelFree(
      lavInspect(object, "free", list.by.group = FALSE),
    ...
    if (modelType %in% c("cfa", "sem")) {
      ...
      if ("nu" %in% names(est) && !is.null(est$nu)) {
        TY <- mapply(FUNV2,
          x = lapply(free[names(free) == "nu"], as.vector),
          y = lapply(est[names(est) == "nu"], as.vector), z = TY, pttemp = ptg, MoreArgs = list(lhstemp = indLab, optemp = "~1"), SIMPLIFY = FALSE
        )
      } else {
        ny <- nrow(est[["lambda"]])
        TY <- mapply(FUNV2, x = rep(list(rep(NA, ny)), ngroups), y = rep(list(rep(
          0,
          ny
        )), ngroups), z = TY, pttemp = ptg, MoreArgs = list(lhstemp = indLab, optemp = "~1"), SIMPLIFY = FALSE)
      }
    ...
    if ("alpha" %in% names(est) && !is.null(est$alpha)) {
      AL <- mapply(FUNV2,
        x = lapply(free[names(free) == "alpha"], as.vector),
        y = lapply(est[names(est) == "alpha"], as.vector), z = AL, pttemp = ptg, MoreArgs = list(lhstemp = NULL, optemp = "~1"), SIMPLIFY = FALSE
      )
    } else {
      p <- ncol(est[["psi"]])
      if (modelType == "path") {
        AL <- mapply(FUNV2, x = rep(list(rep(NA, p)), ngroups), y = rep(list(rep(
          0,
          p
        )), ngroups), z = AL, pttemp = ptg, MoreArgs = list(lhstemp = indLab, optemp = "~1"), SIMPLIFY = FALSE)
      } else {
