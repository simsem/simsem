# findPower: Find a value of a given independent variable that provides a given value of power. This function can handle multiple ivs by split data.
...
findPower <- function(powerTable, iv, power) {
	...
	if(length(grep("iv.", iv)) != 0) iv <- gsub("iv.", "", iv)
	temp <- grep(iv, ivName)
	if(length(temp) == 0) stop("Cannot find the specified target column")
	iv <- temp
	if(length(ivName) == 1) {
...
# findTargetPower: Find a value of a given independent variable that provides a given value of power. This function can handle only one independent variable.
