# getPower:	automatically find the power for all values in the range of given varying parameters or for a set of given value of varying parameters

getPower <- function(simResult, alpha = 0.05, contParam = NULL, powerParam = NULL, nVal = NULL, pmMCARval = NULL, pmMARval = NULL, paramVal = NULL) {
	...
	if(any(condition)) {
		...
		if(!is.null(paramVal)) {
			if(is(paramVal, "list")) {
				if(is.null(names(paramVal))) names(paramVal) <- contParam
		...
		pow <- continuousPower(object, length(object@n) > 1, length(object@pmMCAR) > 1, length(object@pmMAR) > 1, contParam = contParam, alpha = alpha, powerParam = powerParam, pred = pred)
		return(pow)
	} else {
...
# continuousPower: Function to calculate power with continously varying parameters
# Will calculate power over continuously varying n, percent missing, or parameters

continuousPower <- function(simResult, contN = TRUE, contMCAR = FALSE, contMAR = FALSE, contParam = NULL, alpha = 0.05, powerParam = NULL, pred = NULL) {
    ...
    if (contN) {
        ...
        x <- cbind(x, object@n)
        if(predDefault) pred$N <- min(object@n):max(object@n)
    }
    if (contMCAR) {
        ...
        if(predDefault) pred$MCAR <- seq(min(object@pmMCAR), max(object@pmMCAR), by = 0.01)
    ...
    if (contMAR) {
        ...
        if(predDefault) pred$MAR <- seq(min(object@pmMAR), max(object@pmMAR), by = 0.01)
