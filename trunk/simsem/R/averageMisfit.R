...
setMethod("popMisfit", signature(param = "SimRSet", misspec = "SimRSet"), definition = function(param, misspec, dfParam=NULL, fit.measures="all") {
	...
	if(!(all(is.finite(misspecMacs$CM)) && all(eigen(misspecMacs$CM)$values > 0))) stop("The misspecification set is not valid.")
    if(!(all(is.finite(paramMacs$CM)) && all(eigen(paramMacs$CM)$values > 0))) stop("The real parameter set is not valid") 
...
setMethod("popMisfit", signature(param = "MatrixSet", misspec = "MatrixSet"), definition = function(param, misspec, dfParam=NULL, fit.measures="all") {
	if(!validateObject(param)) stop("The set of actual parameters is not valid.")
	if(!validateObject(misspec)) stop("The set of misspecified paramters is not valid.")
...
setMethod("popMisfit", signature(param = "SimSet", misspec = "SimMisspec"), definition = function(param, misspec, dfParam=NULL, fit.measures="all", equalCon=new("NullSimEqualCon")) {
	...
	if(is.null(dfParam)) {
		...
		if(!isNullObject(equalCon)) nFree <- nFree + countFreeParameters(equalCon)
...
popMisfitMACS <- function(paramM, paramCM, misspecM, misspecCM, dfParam=NULL, fit.measures="all") {
	if(fit.measures=="all") {
		...
		if(is.null(dfParam)) fit.measures <- fit.measures[1:2]
	}
	p <- length(paramM)
