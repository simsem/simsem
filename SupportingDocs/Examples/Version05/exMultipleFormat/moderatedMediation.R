...
a1.amb <- .3 # effect of X1 when W == 0
a2.amb <- .3 # effect of X2 when W == 0
## how much "a" paths are moderated by W
a1.unamb <- .3 # effect of (X1 * W) dummy-code interaction
a2.unamb <- .3 # effect of (X2 * W) dummy-code interaction
...
b <- .3 # effect of M on Y
c1.amb <- .3 # direct effect of X1 on Y
c2.amb <- .3 # direct effect of X2 on Y
...
simMod <- model.path(BE = endoPaths, RPS = residCor, KA = exoPaths,
                     indLab = c("M","Y"), covLab = names(dummies),
                     con = userParams)
simMod # kappa gets stored as gamma because the LISREL parameterization
## only allows observed variables (and factors) to be predicted by factors.
## The intercepts/means were also arbitrarily set to 0. Does not affect results.
...
out <- sim(nRep = 10, n = N, model = simMod, covData = covData)
