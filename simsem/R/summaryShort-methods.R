...
setMethod("summaryShort",
  ...
  definition = function(object, alpha = 0.05, digits = 3) {
    ...
    if (object@paramOnly) {
      cat(
        "This object contains only real and misspecified parameter values.\n",
      ...
      if (length(unique(object@pmMCAR)) > 1) {
        cat(paste(
          "Percent Completely Missing at Random:",
      ...
      if (!is.null(object@paramValue)) {
        ...
        if ((ncol(object@coef) != length(targetVar)) ||
          ...
          cat(
            "NOTE: The data generation model is not the same as the analysis model. ",
