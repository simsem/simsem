...
setMethod("summary", signature = "SimResult", definition = function(object, digits = 3, 
    ...
    if(!object@paramOnly) {
		...
		cat(paste("   2.", "At least one SE were negative or NA", "=", sum(object@converged == 2), "\n"))
		cat(paste("   3.", "At least one variance estimates were negative", "=", sum(object@converged == 3), "\n"))
		cat(paste("   4.", "Nonconvergent results from multiple imputation", "=", sum(object@converged == 4), "\n"))
		if (length(unique(object@n)) > 1) 
