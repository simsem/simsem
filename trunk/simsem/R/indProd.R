...
# Argument:
#	data: target data
# 	var1: Varible names or the position of indicators in the first latent variable
#	var2: Variable names or the position of indicators in the second latent variable
#	match:	If TRUE, using match-paired approach. If FALSE, using all possible products
#	meanC:	If TRUE, mean center the indicators of latent variables
...
indProd <- function(data, var1, var2, match=TRUE, meanC=TRUE, residualC=FALSE, doubleMC=TRUE, namesProd=NULL) {
	...
	if(all(is.numeric(var2))) var2 <- colnames(data)[var2]
	dat1 <- data[,var1]
	dat2 <- data[,var2]
	if(meanC) {
