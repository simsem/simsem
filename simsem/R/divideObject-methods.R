...
setMethod("divideObject", signature(object = "vector", constant = "numeric"), 
    definition = function(object, constant) {
...
setMethod("divideObject", signature(object = "matrix", constant = "numeric"), 
    definition = function(object, constant, correlation = FALSE) {
        if (isNullObject(object)) {
...
setMethod("divideObject", signature(object = "MatrixSet", constant = "numeric"), 
    definition = function(object, constant) {
        ...
        Output <- new("MatrixSet", modelType = object@modelType, LY = LY, VTE = VTE, 
            RTE = RTE, VY = VY, TY = TY, MY = MY, BE = BE, VPS = VPS, RPS = RPS, 
            VE = VE, AL = AL, ME = ME, LX = LX, VTD = VTD, RTD = RTD, VX = VX, TX = TX, 
            MX = MX, GA = GA, VPH = VPH, RPH = RPH, KA = KA, RTH = RTH)
        return(Output)
    }) 
