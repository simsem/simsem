...
getKeywords <- function() {
    ...
    indicatorVar <- c(VX, VY, "indVar", "ind.var", "indicator.var", "indicatorVar", 
        "Variance of Indicators")
    indicatorMean <- c(MX, MY, "indMean", "ind.mean", "indicator.mean", "indicatorMean", 
        "means of Indicators")
    facCov <- c(PS, PH, "latent.cov", "latentCov", "factor.cov", "factorCov", "faccov", 
        "factor covariance")
    facCor <- c(RPS, RPH, "latent.cor", "latentCor", "factor.cor", "factorCor", "faccor", 
        "factor correlation")
    intercept <- c(TY, TX, "intercept", "measurement intercept")
    facMean <- c(ME, KA, AL, "factor.mean", "factor mean")
    facVar <- c(VE, VPS, VPH, "latent.var", "latentVar", "factor.var", "factorVar", 
        "facvar", "factor variance")
    ...
    result <- list(LY = LY, TE = TE, RTE = RTE, PS = PS, RPS = RPS, BE = BE, VY = VY, 
        ...
        facVar = facVar, usedFit = usedFit, usedFitPop = usedFitPop, optMin = optMin, 
        optMax = optMax, optNone = optNone)
    return(result)
} 
