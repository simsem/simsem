simMisspecPath <- function(..., exo = FALSE) {
	...
	if(exo) {
		...
		Output <- new("SimMisspec", BE=BE[[1]], PS=PS[[1]], VPS=VPS[[1]], VE=VE[[1]], AL=AL[[1]], ME=ME[[1]], GA=GA[[1]], PH=PH[[1]], VPH=VPH[[1]], KA=KA[[1]], modelType="Path.exo")	
	} else {
		Output <- new("SimMisspec", BE=BE[[1]], PS=PS[[1]], VPS=VPS[[1]], VE=VE[[1]], AL=AL[[1]], ME=ME[[1]], modelType="Path")
	}
	return(Output)
}
