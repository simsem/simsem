...
AvaluesNested <- matrix(0, 12, 12)
AvaluesNested[1:3, 10] <- c(1, 0.8, 1.2)
AvaluesNested[4:6, 11] <- c(1, 0.8, 1.2)
AvaluesNested[7:9, 12] <- c(1, 0.8, 1.2)
...
popNested <- mxModel("Weak Invariance Model",
    type="RAM",
    mxMatrix(type="Full", nrow=12, ncol=12, values=AvaluesNested, free=Afree, labels=Alabels, byrow=TRUE, name="A"),
...
AvaluesParent[1:3, 10] <- c(1, 0.8, 1.2)
AvaluesParent[4:6, 11] <- c(1, 0.4, 1.6)
AvaluesParent[7:9, 12] <- c(1, 1.2, 0.8)

popParent <- mxModel("Configural Invariance Model",
    type="RAM",
    mxMatrix(type="Full", nrow=12, ncol=12, values=AvaluesParent, free=Afree, byrow=TRUE, name="A"),
    mxMatrix(type="Symm", nrow=12, ncol=12, values=Svalues, free=Sfree, byrow=TRUE, name="S"),
    mxMatrix(type="Full", nrow=9, ncol=12, free=FALSE, values=Fvalues, byrow=TRUE, name="F"),
    mxMatrix(type="Full", nrow=1, ncol=12, values=rep(0, 12), free=c(rep(TRUE, 9), rep(FALSE, 3)), name="M"),
    mxExpectationRAM("A","S","F","M", dimnames=c(paste0("y", 1:9), "f1", "f2", "f3"))
...
outDatNestedModParent <- sim(1000, n = 200, popParent, generate = popNested, mxFit=TRUE)
...
outDatParentModNested <- sim(1000, n = 200, popNested, generate = popParent, mxFit=TRUE)
outDatParentModParent <- sim(1000, n = 200, popParent, generate = popParent, mxFit=TRUE)
