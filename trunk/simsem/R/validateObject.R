validateObject <- function(object, detail = FALSE) {
	...
	if(object@modelType == "Path" | object@modelType == "Path.exo" | object@modelType == "SEM" | object@modelType == "SEM.exo") {
		if(validatePath(object@BE, object@VE, object@VE) == FALSE) return(FALSE)
	}
	if(object@modelType == "CFA" | object@modelType == "SEM" | object@modelType == "SEM.exo") {
		if(validateCovariance(object@VTE, object@RTE, object@VY) == FALSE) return(FALSE)
		if(validatePath(object@LY, object@VE, object@VY) == FALSE) return(FALSE)
	}
	if(object@modelType == "Path.exo" | object@modelType == "SEM.exo") {
		if(validateCovariance(object@VPH, object@RPH) == FALSE) return(FALSE)
		if(validatePath(object@GA, object@VPH, object@VE) == FALSE) return(FALSE)
	}
	if(object@modelType == "SEM.exo") {
		if(validateCovariance(object@VTD, object@RTD, object@VX) == FALSE) return(FALSE)
		if(validatePath(object@LX, object@VPH, object@VX) == FALSE) return(FALSE)
