...
simSetCFA <- function(...) { 
	...
	if(10 %in% position) {
		...
		ifelse(5 %in% position, VY <- List[position == 5], ifelse(isNullObject(VTE[[1]]), { VY <- list(freeVector(1, ni)); comment(VY[[1]]) <- "default"}, VY <- list(new("NullSimVector"))))
	}
	if(11 %in% position) {
		PS <- List[position == 11]
		...
		ifelse(9 %in% position, VE <- List[position == 9], { VE <- list(constantVector(1, nk)); comment(VE[[1]]) <- "default"})
	}
	ifelse(8 %in% position, MY <- List[position == 8], MY <- list(new("NullSimVector")))
	ifelse(6 %in% position, TY <- List[position == 6], ifelse(isNullObject(MY[[1]]), { TY <- list(freeVector(0, ni)); comment(TY[[1]]) <- "default"}, TY <- list(new("NullSimVector"))))
	ifelse(7 %in% position, ME <- List[position == 7], { ME <- list(constantVector(0, nk)); comment(ME[[1]]) <- "default"})
