# reassignNames: Match the rownames of the equality constraint, check whether
# it match any model matrices, and substitute with an appropriate matrix name.
...
reassignNames <- function(modelType, Name) {
    ...
    if (modelType == "CFA") {
        keywords <- list(W$loading, W$errorCor, W$facCor, W$errorVar, W$indicatorVar, 
            W$intercept, W$facMean, W$indicatorMean, W$facVar, W$errorCov, W$facCov)  # 11 total
