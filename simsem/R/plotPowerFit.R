...
#'
#' plotPowerFit(Output.ALT,
#'   nullObject = Output.NULL,
#'   usedFit = c("RMSEA", "CFI", "TLI", "SRMR")
#' )
#' }
#'
#' @export
plotPowerFit <- function(
  altObject, nullObject = NULL, cutoff = NULL, usedFit = NULL,
  ...
  plotPowerFitDf(altObject@fit,
    nullObject = nullObject, cutoff = cutoff, usedFit = usedFit,
...
plotPowerFitDf <- function(
  ...
  if (is.null(x)) {
    if (is.null(nullObject)) {
      ...
      plotOverHist(altObject, nullObject,
        cutoff = cutoff, usedFit = usedFit,
        alpha = alpha
      )
    ...
    if (logistic & (!is.null(nullObject) | !is.null(cutoff))) {
      plotLogisticFit(altObject,
        nullObject = nullObject, cutoff = cutoff,
        usedFit = usedFit, x = x, xval = xval, alpha = alpha, useContour = useContour
      )
    } else {
      plotScatter(altObject,
        nullObject = nullObject, cutoff = cutoff, usedFit = usedFit,
        x = x, alpha = alpha
      )
...
#' @keywords internal
plotOverHist <- function(
  altObject, nullObject, cutoff = NULL, usedFit = NULL, alpha = 0.05,
  ...
  for (i in 1:length(common.name)) {
    ...
    overlapHist(nullObject[, i], altObject[, i],
      main = common.name[i], xlab = "Value",
    ...
    legend(position, c("Null", "Alternative"), cex = 1, bty = "n", fill = c(
      "yellow",
      "skyblue"
    ))
  }
...
plotLogisticFit <- function(
  ...
  if (is.null(cutoff)) {
    ...
    cutoff <- getCutoffDataFrame(
      object = nullFit, alpha = alpha, revDirec = FALSE, usedFit = usedFit,
...
plotScatter <- function(
  ...
  for (i in 1:length(usedFit)) {
    ...
    plotIndividualScatter(altObject[, usedFit[i]],
      nullVec = nullVec, cutoff = temp,
      x, main = usedFit[i]
    )
  }
  if (length(usedFit) > 1) {
...
plotIndividualScatter <- function(altVec, nullVec = NULL, cutoff = NULL, x, main = NULL) {
  ...
  plot(c(min(x), max(x)), c(minAll, maxAll),
    type = "n", main = main, xlab = colnames(x),
    ylab = "Value"
  )
...
#' @keywords internal
overlapHist <- function(
  a, b, colors = c("red", "blue", "purple"), breaks = NULL,
