...
setMethod("makeLabels", signature = "SimSet", definition = function(object, 
    ...
    return(new("SimGenLabels", LY = LY, TE = TE, RTE=RTE, VTE=VTE, VY=VY, BE = BE, PS = PS, RPS=RPS, VPS=VPS, VE=VE, AL = AL, ME=ME, TY = TY, MY=MY, LX = LX, TD = TD, RTD=RTD, VTD=VTD, VX=VX, TX = TX, MX=MX, GA = GA, PH = PH, RPH=RPH, VPH=VPH, KA = KA, TH = TH, RTH=RTH, modelType = object@modelType))
})
