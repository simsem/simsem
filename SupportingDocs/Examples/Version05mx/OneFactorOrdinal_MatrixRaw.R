...
dat1 <- generateOpenMx(oneFactorThresholdModel, n=500)
dat2 <- generateOpenMx(oneFactorThresholdFit, n=500)

implied <- getImpliedStatML(oneFactorThresholdFit)
set.seed(4321)
continuousData <- mvrnorm(n=nSubjects,implied[[1]],implied[[2]])
# Simulate multivariate normal data
# -------------------------------------

quants<-quantile(continuousData[,1],  probs = c((1:nThresholds)/(nThresholds+1)))
dat3<-matrix(0,nrow=nSubjects,ncol=nVariables)
for(i in 1:nVariables)
{
dat3[,i] <- cut(as.vector(continuousData[,i]),c(-Inf,quants,Inf))
}
dat3 <- mxFactor(as.data.frame(dat3),levels=c(1:(nThresholds+1)))
# Make the ordinal variables into 
# R factors
# -------------------------------------

fruitynames<-paste("banana",1:nVariables,sep="")
names(dat3)<-fruitynames

out1 <- analyzeOpenMx(oneFactorThresholdModel, dat1)
out2 <- analyzeOpenMx(oneFactorThresholdModel, dat2)
out3 <- analyzeOpenMx(oneFactorThresholdModel, ordinalData)
...
script <- "
f1 =~ banana1 + banana2 + banana3 + banana4 + banana5
"
fitMeasures(cfa(script, data=dat1, std.lv=TRUE))
fitMeasures(cfa(script, data=dat2, std.lv=TRUE))


x1 <- mxRun(oneFactorThresholdModel)
oneFactorThresholdModel2 <- oneFactorThresholdModel
oneFactorThresholdModel2@data <- mxData(ordinalData, type="raw")
x2 <- mxRun(oneFactorThresholdModel2)
oneFactorThresholdModel3 <- oneFactorThresholdModel
oneFactorThresholdModel3@data <- mxData(dat2, type="raw")
x3 <- mxRun(oneFactorThresholdModel3)

round(fitMeasuresOpenMx(out1), 3)
round(fitMeasuresOpenMx(out2), 3)
round(fitMeasuresOpenMx(out3), 3)
