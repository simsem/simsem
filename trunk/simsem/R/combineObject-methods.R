...
setMethod("combineObject", signature(object1 = "SimMatrix", object2 = "SimMatrix"), 
...
setMethod("combineObject", signature(object1 = "SimVector", object2 = "SimVector"), 
    definition = function(object1, object2) {
        Labels1 <- object1@param
        Labels2 <- object2@param
...
setMethod("combineObject", signature(object1 = "vector", object2 = "vector"), 
    definition = function(object1, object2) {
        if (isNullObject(object1)) {
            ...
            if (isNullObject(object2)) {
                ...
                ifelse(length(object1) == length(object2), return(object1 + object2), 
                  stop("Length of vectors are not equal."))
            }
        }
    })

setMethod("combineObject", signature(object1 = "matrix", object2 = "matrix"), 
    definition = function(object1, object2, correlation = FALSE) {
        if (isNullObject(object1)) {
            if (isNullObject(object2)) {
...
setMethod("combineObject", signature(object1 = "MatrixSet", object2 = "MatrixSet"), 
    definition = function(object1, object2) {
        ...
        Output <- new("MatrixSet", modelType = object1@modelType, LY = LY, VTE = VTE, 
            TE = TE, RTE = RTE, VY = VY, TY = TY, MY = MY, BE = BE, VPS = VPS, PS = PS, 
            RPS = RPS, VE = VE, AL = AL, ME = ME, LX = LX, VTD = VTD, TD = TD, RTD = RTD, 
            VX = VX, TX = TX, MX = MX, GA = GA, VPH = VPH, PH = PH, RPH = RPH, KA = KA, 
            TH = TH, RTH = RTH)
        return(Output)
    })

setMethod("combineObject", signature(object1 = "SimParam", object2 = "list"), 
    definition = function(object1, object2) {
        ...
        if (modelType == "CFA") {
            ...
            if (!isNullObject(object1@LY)) 
                object1@LY[is.na(object1@LY)] <- (object2$lambda[name.indicator, 
                  name.factor])[is.na(object1@LY)]
            ...
            e.list <- (nx + 1):(nx + ny)
            if (!isNullObject(object1@PS)) 
                object1@PS[is.na(object1@PS)] <- ((object2$psi[name.indicator, name.indicator])[e.list, 
                  e.list])[is.na(object1@PS)]
            if (!isNullObject(object1@AL)) 
                object1@AL[is.na(object1@AL)] <- ((object2$alpha[name.indicator, 
                  ])[e.list])[is.na(object1@AL)]
            if (!isNullObject(object1@BE)) 
                object1@BE[is.na(object1@BE)] <- ((object2$beta[name.indicator, name.indicator])[e.list, 
                  e.list])[is.na(object1@BE)]
            if (!isNullObject(object1@PH)) 
                object1@PH[is.na(object1@PH)] <- ((object2$psi[name.indicator, name.indicator])[k.list, 
                  k.list])[is.na(object1@PH)]
            if (!isNullObject(object1@KA)) 
                object1@KA[is.na(object1@KA)] <- ((object2$alpha[name.indicator, 
                  ])[e.list])[is.na(object1@KA)]
            if (!isNullObject(object1@GA)) 
                object1@GA[is.na(object1@GA)] <- ((object2$beta[name.indicator, name.indicator])[e.list, 
                  k.list])[is.na(object1@GA)]
            ...
            e.list <- (nk + 1):(nk + ne)
            if (!isNullObject(object1@PS)) 
                object1@PS[is.na(object1@PS)] <- ((object2$psi[name.factor, name.factor])[e.list, 
                  e.list])[is.na(object1@PS)]
            if (!isNullObject(object1@AL)) 
                object1@AL[is.na(object1@AL)] <- ((object2$alpha[name.factor, ])[e.list])[is.na(object1@AL)]
            if (!isNullObject(object1@BE)) 
                object1@BE[is.na(object1@BE)] <- ((object2$beta[name.factor, name.factor])[e.list, 
                  e.list])[is.na(object1@BE)]
            if (!isNullObject(object1@PH)) 
                object1@PH[is.na(object1@PH)] <- ((object2$psi[name.factor, name.factor])[k.list, 
                  k.list])[is.na(object1@PH)]
            if (!isNullObject(object1@KA)) 
                object1@KA[is.na(object1@KA)] <- ((object2$alpha[name.factor, ])[e.list])[is.na(object1@KA)]
            if (!isNullObject(object1@GA)) 
                object1@GA[is.na(object1@GA)] <- ((object2$beta[name.factor, name.factor])[e.list, 
                  k.list])[is.na(object1@GA)]
            x.list <- 1:nx
            y.list <- (nx + 1):(nx + ny)
            if (!isNullObject(object1@LY)) 
                object1@LY[is.na(object1@LY)] <- ((object2$lambda[name.indicator, 
                  name.factor])[y.list, e.list])[is.na(object1@LY)]
            if (!isNullObject(object1@TE)) 
                object1@TE[is.na(object1@TE)] <- ((object2$theta[name.indicator, 
                  name.indicator])[y.list, y.list])[is.na(object1@TE)]
            if (!isNullObject(object1@TY)) 
                object1@TY[is.na(object1@TY)] <- ((object2$nu[name.indicator, ])[y.list])[is.na(object1@TY)]
            if (!isNullObject(object1@LX)) 
                object1@LX[is.na(object1@LX)] <- ((object2$lambda[name.indicator, 
                  name.factor])[x.list, k.list])[is.na(object1@LX)]
            if (!isNullObject(object1@TD)) 
                object1@TD[is.na(object1@TD)] <- ((object2$theta[name.indicator, 
                  name.indicator])[x.list, x.list])[is.na(object1@TD)]
            ...
            if (!isNullObject(object1@TH)) 
                object1@TH[is.na(object1@TH)] <- ((object2$theta[name.indicator, 
                  name.indicator])[x.list, y.list])[is.na(object1@TH)]
        } else {
            stop("something wrong!")
        }
        return(new("SimRSet", modelType = object1@modelType, PS = object1@PS, AL = object1@AL, 
            BE = object1@BE, PH = object1@PH, KA = object1@KA, GA = object1@GA, LY = object1@LY, 
            TE = object1@TE, TY = object1@TY, LX = object1@LX, TD = object1@TD, TX = object1@TX, 
            TH = object1@TH))
    }) 
