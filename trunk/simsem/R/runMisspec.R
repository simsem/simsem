...
runMisspec <- function(object, misspec, SimEqualCon = new("NullSimEqualCon")) {
	...
	if(isNullObject(SimEqualCon)) {
		...
		if(misspec@misBeforeFill & misspec@conBeforeMis & SimEqualCon@conBeforeFill) {
			...
			# 3) fillBefore, Con, AddMis
			# misspec@misBeforeFill=F & misspec@conBeforeMis=T SimEqualCon@conBeforeFill=F; FTF
			paramSet <- run(object, SimEqualCon, makeList = TRUE)
			Output1 <- paramSet[[1]]
			param <- combineObject(paramSet[[2]], Mis)
			Output2 <- fillParam(param, object@modelType)			
			...
			# 4) fillBefore, AddMis, Con
			# isBeforeFill=F & misspec@conBeforeMis=F SimEqualCon@conBeforeFill=F; FFF
			paramSet <- run(object, makeList = TRUE)
			Output1 <- paramSet[[1]]
			param <- combineObject(paramSet[[1]], Mis)
			...
			# 5) AddMis, Con, fill
			# misspec@misBeforeFill=T & misspec@conBeforeMis=F SimEqualCon@conBeforeFill=T; TFT
			paramSet <- run(object, makeList = TRUE)
			Output1 <- paramSet[[1]]
			param <- combineObject(paramSet[[2]], Mis)
			param <- constrainMatrices(param, SimEqualCon)
			...
			# 6) AddMis, fill, Con
			# misspec@misBeforeFill=T & misspec@conBeforeMis=F SimEqualCon@conBeforeFill=F; TFF ; No FFT and TTF
			paramSet <- run(object, makeList = TRUE)
			Output1 <- paramSet[[1]]
			param <- combineObject(paramSet[[2]], Mis)
