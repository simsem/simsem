...
pValueVector <- function(
  ...
  if (length(target) == 1) {
    if (is.null(x)) {
      ...
      if (condCutoff) {
        return(pValueCondCutoff(target, dist,
          revDirec = revDirec, x = x,
          xval = xval, df = df
        ))
      } else {
        return(pValueVariedCutoff(rep(target, length(dist)), dist,
          revDirec = revDirec,
          x = x, xval = xval
        ))
      }
...
pValueDataFrame <- function(
  ...
  if (asLogical) {
    ...
    for (i in 1:numVar) {
      result[i] <- pValueVector(target[i], dist[, i], revDirec[i],
        x = x, xval = xval,
        df = df
      )
    }
...
#'
#' @keywords internal
pValueCondCutoff <- function(
  target, dist, revDirec = FALSE, x = NULL, xval = NULL,
  df = 0
) {
  if (!is.matrix(x)) {
    x <- as.matrix(x)
...
whichMonotonic <- function(vec, ord = NULL, anchor = NULL) {
  ...
  p <- length(vec)
  if (is.null(anchor)) {
    anchor <- round((p / 2) + 0.001)
  } # Add a small decimal so that 0.5 will be rounded as 1 instead of 0
...
#'
#' @keywords internal
pValueVariedCutoff <- function(
  cutoff, obtainedValue, revDirec = FALSE, x = NULL,
  xval = NULL
) {
