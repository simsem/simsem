...
# that visualize power

setMethod("plotPowerFit", signature(altObject = "data.frame", nullObject = "vector"), 
    definition = function(altObject, nullObject, usedFit = NULL) {
        plotCutoff(altObject, nullObject, usedFit = usedFit)
    })

setMethod("plotPowerFit", signature(altObject = "SimResult", nullObject = "vector"), 
    definition = function(altObject, nullObject, usedFit = NULL) {
        altObject <- clean(altObject)
        plotCutoff(altObject@fit, nullObject, usedFit = usedFit)
    })

setMethod("plotPowerFit", signature(altObject = "data.frame", nullObject = "data.frame"), 
...
setMethod("plotPowerFit", signature(altObject = "SimResult", nullObject = "SimResult"), 
    definition = function(altObject, nullObject, alpha, usedFit = NULL) {
        altObject <- clean(altObject)
